...
const HomologDataCard: React.FC<SelectProcess> = ({
  ...
  const [checkedFormularioPontuacao, setCheckedFormularioPontuacao] =
    useState(false);
  const [checkedDiplomaGraduacao, setCheckedDiplomaGraduacao] = useState(false);
  const [checkedHistoricoGraduacao, setCheckedHistoricoGraduacao] =
    useState(false);
  const [checkedRgCpf, setCheckedRgCpf] = useState(false);
  const [checkedQuitacaoMilitar, setCheckedQuitacaoMilitar] = useState(false);
  const [checkedQuitacaoEleitoral, setCheckedQuitacaoEleitoral] =
    useState(false);
  const [checkedComprovanteResidencia, setCheckedComprovanteResidencia] =
    useState(false);
  const [checkedComprovanteCota, setCheckedComprovanteCota] = useState(false);
  const [checkedComprovanteServidorIfpb, setCheckedComprovanteServidorIfpb] =
    useState(false);
  ...
  const handleAcademicDataChange = (
    event: React.ChangeEvent<HTMLInputElement>,
  ) => {
  ...
  const handleCheckFormularioPontuacao = (
    event: React.ChangeEvent<HTMLInputElement>,
  ) => {
    setCheckedFormularioPontuacao(event.target.checked);
  };

  const handleCheckDiplomaGraduacao = (
    event: React.ChangeEvent<HTMLInputElement>,
  ) => {
    setCheckedDiplomaGraduacao(event.target.checked);
  };

  const handleCheckHistoricoGraduacao = (
    event: React.ChangeEvent<HTMLInputElement>,
  ) => {
  ...
  return (
    <Box sx={{ width: '100%', mb: 3 }}>
      ...
      >
        ...
        <Typography variant="body1" sx={{ mb: 1 }}>
          <strong>
            Formulário de pontuação preenchido e documentos comprobatórios
            (Anexo I)
          </strong>{' '}
          {''}
        </Typography>
        ...
        <Typography variant="body1" sx={{ mb: 1 }}>
          <strong>
            Cópia do diploma ou certificado de conclusão de curso de graduação
        ...
        <Typography variant="body1" sx={{ mb: 1 }}>
          <strong>
            Comprovante de quitação das obrigações militares (se aplicável)
          </strong>{' '}
          {''}
        </Typography>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
      ...
      >
        ...
        <Divider sx={{ mb: 2 }} />
        <Typography variant="body1" sx={{ mb: 1 }}>
          <strong>Declaração de optante por cota de ingresso (Anexo II)</strong>{' '}
          {cota}
        </Typography>
        ...
        <Typography variant="body1" sx={{ mb: 1 }}>
          <strong>Declaração de cota de servidor do IFPB (se aplicável)</strong>{' '}
          {cota}
        </Typography>
